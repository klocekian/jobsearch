...
/**
 * Resolves the profile autofill fields: stored overrides take priority,
 * otherwise they're parsed from the user's default resume. Shared between
...
export async function getAutofillFields(userId: number | null, fallbackEmail?: string): Promise<Record<string, unknown> | { error: string }> {
  ...
  const email = emailMatch?.[0] ?? fallbackEmail ?? "";
